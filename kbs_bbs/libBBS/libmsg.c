...
void msgmail(char *did, const char *buf)
{
    ...
    if ((tfp = fopen(tmpfname, "w"))!=NULL) {
        write_header(tfp, getCurrentUser(),1,NULL,"����ʧ�ܵ���Ϣ",0,0,getSession());
        fprintf(tfp, "\n���%s����Ϣ���ڶԷ��Ѿ����߻�����Ļ�����޷��ʹ�,��������Ϣ����:\n\n%s\n", did, buf);
        fclose(tfp);

        mail_file(getCurrentUser()->userid, tmpfname, getCurrentUser()->userid, "����ʧ�ܵ���Ϣ", BBSPOST_MOVE, NULL);
    }
...
int sendmsgfunc(struct user_info *uentp, const char *msgstr, int mode, int srcpid, session_t* session)
{
    ...
    if ((mode != 3) && (LOCKSCREEN == uin->mode)) {     /* Leeward 98.02.28 */
        strcpy(session->msgerr, "�Է��Ѿ�������Ļ�����Ժ��ٷ������(��)д��...");
...
//       return -1;
//    }
    if ((mode != 3) && (false == canIsend2(session->currentuser,uin->userid))) {     /*Haohmaru.06.06.99.����Լ��Ƿ�ignore */
        strcpy(session->msgerr, "�Է��ܾ��������ѶϢ...");
        msgmail(uident, msgstr);
        return -1;
    }
    if (mode != 3 && uin->pid != 1) {
        if (get_unreadcount(uident) > MAXMESSAGE) {
            strcpy(session->msgerr, "�Է�����һЩѶϢδ���������Ժ��ٷ������(��)д��...");
            msgmail(uident, msgstr);
            return -1;
        }
    }
...
int translate_msg(char* src, struct msghead *head, char* dest,session_t* session)
{
    ...
    switch (head->mode) {
        ...
        case 4:
            ...
            break;
        case 3:
            sprintf(dest, "\033[44%sm\x1b[33mվ���� %16.16s ʱ�㲥\033[37m\033[K\033[m\n", DEFINE(session->currentuser, DEF_HIGHCOLOR)?";1":"", time);
            sprintf(attstr, "\033[44%sm\033[37m", DEFINE(session->currentuser, DEF_HIGHCOLOR)?";1":"");
            break;
        case 1:
            if (!head->sent) {
                sprintf(dest, "\033[44%sm\x1b[36m%-12.12s(%-16.16s) ������\033[37m\033[K\033[m\n", DEFINE(session->currentuser, DEF_HIGHCOLOR)?";1":"", head->id, time);
                sprintf(attstr, "\033[44%sm\033[37m", DEFINE(session->currentuser, DEF_HIGHCOLOR)?";1":"");
            } else {
                sprintf(dest, "\033[44%sm\x1b[37m��(%-16.16s) ����%-12.12s\033[36m\033[K\033[m\n", DEFINE(session->currentuser, DEF_HIGHCOLOR)?";1":"", time, head->id);
...
void mail_msg(int id, struct userec* user,session_t* session)
{
    ...
    now = time(0);
    /* fancy Jan 13 2008, ������ sender/content ��Ѱ, �е�� dirty ... */
    if (id > 0)
        sprintf(title, "[%20.20s] ����ѶϢ���� (��Ѷ��)", ctime_r(&now, timebuf) + 4);
    else if (!id)
        sprintf(title, "[%20.20s] ����ѶϢ����", ctime_r(&now, timebuf) + 4);
    else {
        sprintf(title, "[%20.20s] ����ѶϢ���� (����)", ctime_r(&now, timebuf) + 4);
        id = -id;
    }
