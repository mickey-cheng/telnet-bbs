...
char dirt_str[2][2] = {" ", "*"};
char drawing_str[3][7] = {"̧��", "���", "����"};
char linemode_str[4][7]={"�в�д","�в���","�в�д","�в���"};
char state_str[8][10]={"�༭","��ѡ��","��༭","��ɫճ��", "�������", "GBK�����", "���ʼ�"};
...
BADCMD:
        strcpy(message, "\033[31mδ֪����,��<ESC>h���İ���.\033[0m");
        draw_sbar();
    }
...
void draw_sbar()
{
    ...
    switch (state) {
        case S_EDIT:
            ...
            sprintf(s, "\033[0m%s%s%02d,%02d%s%s%s%s%s%s%s%s%s%s", dirt_str[(anchor_dirt != undo_ptr->dirt)], char_sep, cur_x+1, cur_y+1, char_sep, tmp1, char_sep, get_char_str(&pens[cur_pen][0], 0), drawing_str[drawing], char_sep, linemode_str[linemode], char_sep, state_str[state], char_sep2, message);
            break;
        case S_SELBLK:
        ...
        case S_EDITBLK:
            ...
            break;
        case S_APST:
            sprintf(s, "\033[0m�����ʽ:%s%s%s%s%s", apst_mode_str[apst_mode], char_sep, state_str[state], char_sep2, message);
            break;
        case S_HLPCMD:
...
void help_code(int page)
{
    int code;
    int i,j;
    char* url = "������μ�:http://www.khngai.com/chinese/charmap/tblgbk.php?page=0";
    ...
    prints(url);
    sprintf(message, "\033[0m���Ҽ���ҳ,CTRL_Q�˳�,��%d/%dҳ", page+1, gbk_total_pages);
...
void help_cmd(int page)
{
    ...
    sprintf(message, "\033[0m���Ҽ���ҳ,CTRL_Q�˳�,��%d/%dҳ", page+1, hcmd_total_pages);
    gotoxy2(scr_cols-1, scr_lns-1);
    draw_sbar();
}
...
void _apen()
{
    ...
    if (num_pens == MAX_PENS)
        sprintf(message, "\033[31m10֧������,�޷�������.\033[0m");
    else {
        ci = &graph[cur_y][cur_x];
        assert(!ci->chinese || (ci->chinese && !ci->half));
        ...
        cur_pen = num_pens;
        num_pens++;
        sprintf(message, "\033[0m����%s����Ϊ��ǰ����.", get_char_str(&pens[cur_pen][0],0));
        chkpt_commit();
    }
    draw_sbar();
}
void _dpen()
{
    ...
    if (!cur_pen)
        strcpy(message, "\033[31m��0�ű�����Ƥ��,���ܱ�ɾ��.\033[0m");
    else {
        chkpt_begin(-1, -1, 3, &cur_pen, sizeof(char), &num_pens, sizeof(char), pens, sizeof(pens));
        ...
        if (cur_pen >= num_pens)
            cur_pen = num_pens-1;
        sprintf(message, "\033[0m����%s�ѱ�ɾ��.",s);
        chkpt_commit();
    }
    draw_sbar();
}
...
{
    if (!hcmd_total_pages) {
        strcpy(message, "\033[31m�޷�װ����������ļ�.\033[0m");
        state = prev_state;
        draw_sbar();
        return;
    }
...
void _load()
{
    ...
    while (1) {
        sprintf(message, "\033[0m��ȡ����(CTRL_Qȡ��):%s", save_file);
        draw_sbar();
        n = strlen(save_file);
        ch = getch();
        ...
        if (n < 15 && (isalnum(ch) || ch=='_' || ch=='-' || ch=='.')) {
            ...
            if (!n)
                ...
                anchor_dirt = dirt = 0;
                sprintf(message, "\033[0m�����ļ�%s��ȡ�ɹ�.", save_file);
                ...
                strcpy(message, "\033[31m��ȡʧ��.\033[0m");
            break;
        }
    }
    draw_sbar();
...
void _save()
{
    ...
    while (1) {
        sprintf(message, "\033[0m�������(CTRL_Qȡ��):%s", save_file);
        draw_sbar();
        n = strlen(save_file);
        ch = getch();
        if (ch == ESC && getch1() == BRK && getch1()=='3' && getch1()=='~')
        ...
        if (n < 15 && (isalnum(ch) || ch=='_' || ch=='-' || ch=='.')) {
            ...
            if (!n)
                ...
                anchor_dirt = dirt;
                sprintf(message, "\033[0m�����ļ�%s����ɹ�.", save_file);
            } else
                strcpy(message, "\033[31m����ʧ��.\033[0m");
            break;
        }
    }
    draw_sbar();
...
void _export()
{
    ...
    while (1) {
        sprintf(message, "\033[0m������ļ�(CTRL_Qȡ��):%s", export_file);
        draw_sbar();
        ...
        if (n < 15 && (isalnum(ch) || ch=='_' || ch=='-' || ch=='.')) {
            ...
            if (!n)
                ...
                sprintf(message, "\033[0m�ļ�%s����ɹ�.", export_file);
            else
                strcpy(message, "\033[31m���ʧ��.\033[0m");
            break;
        }
...
void _redo()
{
    ...
    int r1, r2;
    if (!redo(&r1, &r2))
        strcpy(message, "\033[31m�޷�����REDO.\033[0m");
    else {
        if (r1!=-1 && r2!=-1) {
            save_cur();
            for (i=r1; i<=r2; i++) {
...
void _undo()
{
    ...
    int r1, r2;
    if (!undo(&r1, &r2))
        strcpy(message, "\033[31m�޷�����UNDO.\033[0m");
    else {
        if (r1!=-1 && r2!=-1) {
            save_cur();
            for (i=r1; i<=r2; i++) {
...
void _sskey()
{
    ...
    *ci1 = *ci;
    ...
    sprintf(message, "\033[0m%s����Ϊ��%d�ſ�ݼ�.", get_char_str(&sc_keys[n][0],0), n);
    chkpt_commit();
    draw_sbar();
}
void _lskey()
...
void _copy()
{
    ...
    if (ci->chinese)
        char_copy[1] = *(ci+1);
    sprintf(message, "\033[0m�ַ�%s������.", get_char_str(ci,1));
    draw_sbar();
}
void _pst()
{
    ...
    switch (copy_type) {
        case CP_CHAR:
            ...
            prints(get_row_str(cur_x, cur_y, 0));
            load_cur();
            sprintf(message, "\033[0m�ַ�%s��ճ��.", get_char_str(ci,1));
            dirt++;
            chkpt_commit();
            break;
        case CP_BLOCK:
    ...
    while (1) {
        ...
        if ((ch>='0' && ch<='9') || (ch>='a' && ch<='f') || (ch>='A' && ch<='F')) {
            if ((!n && ch>='8') || n) {
                ...
                draw_sbar();
                if (n==4) {
                    sprintf(message, "\033[0mGBK��%s�������.", gbk);
...
void _blk1()
{
    ...
    blk_x1 = blk_x2 = cur_x;
    blk_y1 = blk_y2 = cur_y;
    strcpy(message, "\033[0m�����趨��,CTRL_Kȷ��,CTRL_Q�˳�.");
...
void _blk2()
{
    ...
    strcpy(message, "\033[0m�����ÿ������,CTRL_Q����,<ESC>h����.");
    clr_reverse();
    draw_graph();
    draw_sbar();
}
...
void _apst()
{
    ...
    chkpt_begin(0, scr_lns-2, 3, &cur_x, sizeof(char), &cur_y, sizeof(char), graph, sizeof(graph));
    apst_step = AP_DATA;
    strcpy(message, "\033[0mճ����ɫ�ı�,CTRL_O�ı������ʽ,CTRL_Q����.");
    draw_sbar();
}
void _amode()
{
        ...
        case ML_ADDR:
            if (!strchr(mail_addr, '@')) {
                if (HAS_PERM(getCurrentUser(),PERM_DENYMAIL)) {
                    strcpy(message, "\033[31m����Ȩ����վ�û�����.\033[0m");
                    draw_sbar();
                    break;
                }
                if (getuser(mail_addr, &u) == 0) {
                    strcpy(message, "\033[31m�����ʹ�����ʺ�.\033[0m");
                    draw_sbar();
                    break;
                }
            }
...
int editor_main()
{
    ...
    //tty_raw(0);
    clr_scr();
    sprintf(message, "��ӭʹ��%s,<ESC>h����.", EDITOR_NAME);
    draw_sbar();
    gotoxy(0,0);
    state=S_EDIT;
    while (1) {
