...
#define UPDATE_UTMP_STR(field,entp) { if (CHECK_UENT((entp).uid)) \
    ...
    int my_f_rm(char *fname);/*��װf_rm,���ڶ��屸�ݵ��ļ�*/
    char* gettmpfilename(char *name,const char *format,...);    /* �� tmp Ŀ¼��ɢ�г���ʱ�ļ���, ����ʱ���� NULL ... */
    ...
    char *sethomepath(char *buf, const char *userid); /* ȡ ĳ�û� ��home */

    char *setmailfile(char *buf, const char *userid, const char *filename);     /* ȡĳ�û�mail�ļ� ·�� */
    char *setmailpath(char *buf, const char *userid); /* ȡ ĳ�û� ��mail */
    char *setbpath(char *buf, const char *boardname); /* ȡĳ�� ·�� */
    char *setbfile(char *buf,const char *boardname,const char *filename); /* ȡĳ�����ļ� */
    void RemoveMsgCountFile(const char *userID);
    int bad_user_id(const char *userid);      /* ���.badname�Ƿ�����ע��� */
    int valid_ident(const char *ident);       /* ���Ϸ���ident */
...
#define BBS_RETURN_VOID                     \
    ...
    int get_nextid(const char *boardname);    /*�������������Ų��Զ���һ */
    int get_nextid_bid(int bid);
    void board_update_toptitle(int bid,bool needlock); /*�ı��ö�����*/
...
//    int ExistFavBoard(int idx,session_t* session);
    ...
    void brc_clear_new_flag(unsigned fid, int bid, session_t* session);      /* �������ĵ���ƪ����δ����� */
    void brc_update(const char *userid,session_t* session);      /* ���浱ǰ��brclist���û���.boardrc */
    ...
    int chk_BM_instr(const char *BMstr, const char *bmname);       /*��ѯ�ַ������Ƿ���� bmname */
    int chk_currBM(const char *BMstr, const struct userec *user);  /* ��������İ������� �ж�user�Ƿ��а��� Ȩ�� */
    int chk_BM(const char *bname, const struct userec *user); /* ���ݰ��������ж� user �Ƿ��а���Ȩ�� ... */
    ...
    int deldeny(struct userec *user, char *board, char *uident, int notice_only, int dobmlog, session_t* session);       /* ɾ�� ��ֹPOST�û� */
    ...
    int post_cross(struct userec *user, const struct boardheader *toboard, const char *fromboard, const char *title, const char *filename, int Anony, int in_mail, char islocal, int mode, session_t* session);   /* (�Զ������ļ���) ת�����Զ����� */
    ...
    char* sysconf_relocate(const char *data); /*���¶�λmenuitem������ַ��� */
    ...
    int a_additem(MENU *pm,const char *title,const char *fname,char *host,int port,long attachpos);    /* ����ITEM object,����ʼ�� */
